...
func Point(v int64) predicate.User {
...
func Pin(v string) predicate.User {
	return predicate.User(sql.FieldEQ(FieldPin, v))
}
...
func HasBoothWith(preds ...predicate.Booth) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newBoothStep()
...
// HasChargeRequests applies the HasEdge predicate on the "charge_requests" edge.
func HasChargeRequests() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasChargeRequestsWith applies the HasEdge predicate on the "charge_requests" edge with a given conditions (other predicates).
func HasChargeRequestsWith(preds ...predicate.ChargeRequest) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newChargeRequestsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
