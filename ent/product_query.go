...
func (_q *ProductQuery) QueryBooth() *BoothQuery {
...
func (_q *ProductQuery) QueryTransactions() *TransactionQuery {
...
func (_q *ProductQuery) WithBooth(opts ...func(*BoothQuery)) *ProductQuery {
	query := (&BoothClient{config: _q.config}).Query()
...
func (_q *ProductQuery) WithTransactions(opts ...func(*TransactionQuery)) *ProductQuery {
	query := (&TransactionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := _q.withTransactions; query != nil {
		if err := _q.loadTransactions(ctx, query, nodes,
			func(n *Product) { n.Edges.Transactions = []*Transaction{} },
			func(n *Product, e *Transaction) { n.Edges.Transactions = append(n.Edges.Transactions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProductQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*Product, init func(*Product), assign func(*Product, *Transaction)) error {
	...
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.TransactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_transactions
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_transactions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_transactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
