...
type UserQuery struct {
	...
	withChargeRequests *ChargeRequestQuery
...
// QueryBooth chains the current query on the "booth" edge.
func (_q *UserQuery) QueryBooth() *BoothQuery {
	query := (&BoothClient{config: _q.config}).Query()
...
// QueryChargeRequests chains the current query on the "charge_requests" edge.
func (_q *UserQuery) QueryChargeRequests() *ChargeRequestQuery {
	query := (&ChargeRequestClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBooth:          _q.withBooth.Clone(),
		withTransactions:   _q.withTransactions.Clone(),
		withChargeRequests: _q.withChargeRequests.Clone(),
...
// WithBooth tells the query-builder to eager-load the nodes that are connected to
// the "booth" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBooth(opts ...func(*BoothQuery)) *UserQuery {
	query := (&BoothClient{config: _q.config}).Query()
...
// WithChargeRequests tells the query-builder to eager-load the nodes that are connected to
// the "charge_requests" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithChargeRequests(opts ...func(*ChargeRequestQuery)) *UserQuery {
	query := (&ChargeRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withChargeRequests = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBooth; query != nil {
		if err := _q.loadBooth(ctx, query, nodes, nil,
			func(n *User, e *Booth) { n.Edges.Booth = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withChargeRequests; query != nil {
		if err := _q.loadChargeRequests(ctx, query, nodes,
			func(n *User) { n.Edges.ChargeRequests = []*ChargeRequest{} },
			func(n *User, e *ChargeRequest) { n.Edges.ChargeRequests = append(n.Edges.ChargeRequests, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadBooth(ctx context.Context, query *BoothQuery, nodes []*User, init func(*User), assign func(*User, *Booth)) error {
	...
	query.withFKs = true
	query.Where(predicate.Booth(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BoothColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_booth
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_booth" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_booth" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadChargeRequests(ctx context.Context, query *ChargeRequestQuery, nodes []*User, init func(*User), assign func(*User, *ChargeRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.ChargeRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ChargeRequestsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_charge_requests
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_charge_requests" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_charge_requests" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
