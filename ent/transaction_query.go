...
func (_q *TransactionQuery) QueryBooth() *BoothQuery {
...
func (_q *TransactionQuery) QueryProduct() *ProductQuery {
...
func (_q *TransactionQuery) WithBooth(opts ...func(*BoothQuery)) *TransactionQuery {
	query := (&BoothClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TransactionQuery) WithProduct(opts ...func(*ProductQuery)) *TransactionQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(transaction.FieldQuantity).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldQuantity).
//		Scan(ctx, &v)
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if _q.withUser != nil || _q.withBooth != nil || _q.withProduct != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withBooth; query != nil {
		if err := _q.loadBooth(ctx, query, nodes, nil,
			func(n *Transaction, e *Booth) { n.Edges.Booth = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes, nil,
			func(n *Transaction, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TransactionQuery) loadBooth(ctx context.Context, query *BoothQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Booth)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].booth_transactions == nil {
			continue
		}
		fk := *nodes[i].booth_transactions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "booth_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Product)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].product_transactions == nil {
			continue
		}
		fk := *nodes[i].product_transactions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
